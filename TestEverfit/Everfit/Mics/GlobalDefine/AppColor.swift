import UIKit

class AppColor: NSObject {
    static let appBackground: UIColor = UIColor(named: "AppBackground") ?? .white
    static let selected: UIColor = UIColor(named: "Selected") ?? .white
    static let mainText: UIColor = UIColor(named: "MainText") ?? .white
    static let missedText: UIColor = UIColor(named: "MissedText") ?? .white
    static let subText: UIColor = UIColor(named: "SubText") ?? .white
    static let normalBackground: UIColor = UIColor(named: "NormalBackground") ?? .white
}
